;+
; Split a mscb1 file into several files per hour.
;
; This file should be used by EFW to batch process the data over the whole mission.
;-

pro rbsp_efw_phasef_gen_l1_mscb1_split, file, split_cadence=split_cadence, $
    ...
    pos = strpos(path,'mscb1')
    path = strmid(path,0,pos)+'mscb1_split'+strmid(path,pos+5)
...
;---Read global and variable attributes and time range.
    time_var = 'epoch'
    b1_var = 'mscb1'
    other_vars = ['mscb1_labl','mscb1_unit','mscb1_compno']
...
;---Loop through the file_times, in each loop only read a portion of the data.
    foreach file_time, file_times do begin
        ...
        split_base = apply_time_to_pattern(prefix+'l1_mscb1_%Y%m%d_%H%M_v02.cdf', file_time)
        ...
        foreach var, other_vars, var_id do begin
            data = other_data[var_id]
            vatt = other_vatt[var_id]
            cdf_save_var, var, filename=split_file, value=transpose(data)
            cdf_save_setting, vatt, filename=split_file, varname=var
        endforeach
    endforeach
...
foreach probe, probes do begin
    ...
    ;if probe eq 'a' then start_date = '2012-01-01' else start_date = '2019-03-16'
	start_date = time_double(start_date)

    foreach year, years do begin
        path = in_local_root+'/'+rbspx+'/l1/mscb1/'+year
        files = file_search(path+'/*_v02.cdf')
        foreach file, files do begin
			; Check file date.
			base = file_basename(file)
			file_time = time_double(strmid(base,15,8),tformat='YYYYMMDD')
			if file_time lt start_date then continue

			rbsp_efw_phasef_gen_l1_mscb1_split, file, $
				in_local_root=in_local_root, out_local_root=out_local_root
		endforeach
    endforeach
endforeach
...
file = '/Volumes/data/rbsp/rbspa/l1/mscb1/2013/rbspa_l1_mscb1_20130110_v02.cdf'
; more than 1 hour of data.
file = '/Volumes/data/rbsp/rbspa/l1/mscb1/2013/rbspa_l1_mscb1_20130128_v02.cdf'
rbsp_efw_phasef_gen_l1_mscb1_split, file
end
