...
; v05:
;   1. fix MLT,
;   2. angle_Bo, and the associated flags,
;   3. change epoch units to second.
;   4. the changes made in rbsp_efw_phasef_gen_l3_v04_patch1.pro
;   5. add DOI to global attributes.
;   6. Change vel_gse_LABL_1 to (a) Record variance F, (b) FORMAT 'A9'
;   7. boom_pair_used_for_spinfit changed to support_data
;   8. burst[12]_avail add label "1 means burst data available"
;   9. Fix deprecated URL.
;-

pro rbsp_efw_phasef_gen_l3_v05_skeleton, file
...
;---Gloabel attributes.
    ...
    ; DOI.
    gatts['DOI'] = rbsp_efw_phasef_get_doi(probe=probe)
...
;---Unit for epoch.
    ...
    foreach var, vars, var_id do begin
        ...
        cdf_save_setting, 'UNITS', 'UTC (second)', filename=file, varname=var
    endforeach
...
pro rbsp_efw_phasef_gen_l3_v05, date, probe=probe, filename=file, log_file=log_file
...
;---Prepare skeleton.
    skeleton_base = prefix+'efw-l3_00000000_v05.cdf'
...
;---Save the used boom pair.
    ...
    cdf_save_var, bp_var, value=fix(the_boom_pair), filename=file, save_as_one=1
    date_var = 'date'
    settings = dictionary($
        'DEPEND_0', date_var, $
        'FORMAT', 'I2', $
    ...
    cdf_save_var, date_var, value=time_range[0], filename=file, save_as_one=1
    settings = dictionary($
        'FIELDNAM', 'date', $
        'CATDESC', 'date', $
        'VAR_TYPE', 'metadata' )
...
;---Flags and the related vars.
    ...
    diagb_var = prefix+'diagBratio'
    interp_time, diagb_var, common_times
    diag_bratio = get_var_data(diagb_var)
...
;---Save wanted E field as wanted names:
    ...
    angle_min = 15.    ; deg.
    rad = constant('rad')
    limiting_ratio = cos(angle_min*rad)/cos((90-angle_min)*rad)
    byz2bx = abs(diag_bratio)
    bad_index = where(total(byz2bx gt limiting_ratio, 2) ne 0, count)
    e_var = prefix+'e_spinfit_mgse_edotb_v'+the_boom_pair
...
;---Burst 1 and 2 avail flag.
    ...
    foreach var, vars do begin
        ...
        str = strmid(var,5,1)
        vatt = dictionary('VAR_NOTES', 'Burst '+str+' data available flag. 1 means burst data available')
        cdf_save_setting, vatt, varname=var, filename=file
    endforeach
...
;---HSK data.
    ; bias_current
    rbsp_efw_init
    !rbsp_efw.local_data_dir = rbsp_efw_phasef_local_root()+'/'
...
;        path = join_path([root_dir,rbspx,'l3_v05',str_year])
;        base = prefix+'efw-l3_'+time_string(day,tformat='YYYYMMDD')+'_v05.cdf'
;        file = join_path([path,base])
;if file_test(file) eq 1 then continue
;        print, file
;        rbsp_efw_phasef_gen_l3_v05, day, probe=probe, filename=file
;    endforeach
;endforeach
;stop
...
file = join_path([homedir(),'rbsp'+probe+'_efw-l3_v05.cdf'])
rbsp_efw_phasef_gen_l3_v05, date, probe=probe, file=file
end
