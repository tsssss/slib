...
;   Fix MLT, old MLT is -12 h off.
;   Fix diagBratio, have invalid values.
;   Add actual values angle_Bo to flags. Old version does not fill this flag.
;-

pro rbsp_efw_phasef_gen_p4_v05, date, $
...
;---Prepare skeleton.
    skeleton_base = prefix+'efw-p4_00000000_v05.cdf'
    skeleton = join_path([srootdir(),skeleton_base])
    if file_test(skeleton) eq 0 then begin
        errmsg = 'Skeleton file: '+skeleton+' is not found ...'
        lprmsg, errmsg, log_file
        return
    endif
...
;         base = prefix+'efw-p4_'+time_string(day,tformat='YYYYMMDD')+'_v05.cdf'
;         file = join_path([path,base])
; if file_test(file) eq 1 then continue
;         print, file
;         rbsp_efw_phasef_gen_p4_v05, day, probe=probe, filename=file
;     endforeach
; endforeach
; stop
...
date = '2017-09-07'
probe = 'a'
; Aaron's event, c.f. email on 2024-07-03.
date = '2014-01-06'
probe = 'b'

file = join_path([homedir(),'test_level4.cdf'])
tic
rbsp_efw_phasef_gen_p4_v05, date, probe=probe, file=file
toc
end