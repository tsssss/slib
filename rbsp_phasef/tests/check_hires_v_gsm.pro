...
pro check_hires_v_gsm, date, probe=probe, component_index=component_index
    prefix = 'rbsp'+probe+'_'
...
;---Version 1: from spice.
    ...
    tplot_rename, prefix+'state_vel_gsm', prefix+'v_gsm_spice'
    v_gsm_spice = get_var_data(prefix+'v_gsm_spice')
    store_data, prefix+'v_gsm_spice', limits=limits



;---Version 2: from interpolation.
    rbsp_read_sc_vel, full_time_range, probe=probe
    v_gsm = get_var_data(prefix+'v_gsm', times=times, limits=limits)

    ; Spline.
    v_gsm_interp = sinterpol(v_gsm, times, hires_times, /spline)
    store_data, prefix+'v_spline', hires_times, v_gsm_interp, limits=limits
    dv_interp = v_gsm_spice-v_gsm_interp
    store_data, prefix+'dv_spline', hires_times, dv_interp, limits=limits

    ; Linear.
    v_gsm_interp = sinterpol(v_gsm, times, hires_times)
    store_data, prefix+'v_linear', hires_times, v_gsm_interp, limits=limits
    dv_interp = v_gsm_spice-v_gsm_interp
    store_data, prefix+'dv_linear', hires_times, dv_interp, limits=limits

    ; Quadratic.
    v_gsm_interp = sinterpol(v_gsm, times, hires_times, /quadratic)
    store_data, prefix+'v_quadratic', hires_times, v_gsm_interp, limits=limits
    dv_interp = v_gsm_spice-v_gsm_interp
    store_data, prefix+'dv_quadratic', hires_times, dv_interp, limits=limits
    ...
    store_data, prefix+'v'+component+'_spice', hires_times, v_gsm_spice[*,component_index], limits={ytitle:'(km/s)', labels:'SPICE'}

    var_types = ['quadratic','spline','linear']
    foreach var_type, var_types do begin
        in_var = prefix+'dv_'+var_type
        get_data, in_var, times, vec, limits=limits
        out_var = prefix+'dv'+component+'_'+var_type
        store_data, out_var, times, vec[*,component_index], limits={ytitle:'(km/s)', labels:str_cap(var_type)}
    endforeach


    plot_vars = prefix+['dv'+component+'_'+var_types,'v'+component+'_spice']
    plot_labels = 'GSM '+['dV'+component+'!C'+var_types+' - SPICE','V'+component+'!CSPICE']
...
;---Gen plot.
    plot_file = join_path([homedir(),'rbsp_phase_f',$
        prefix+time_string(time_range[0],tformat='YYYY_MMDD')+'_vel_study_v'+component+'_v01.pdf'])
    ...
    tx = tpos[0]
    ty = tpos[3]+ychsz*0.5
    msg = 'RBSP-'+strupcase(probe)+' spacecraft GSM V'+component+' (km/s). '+$
        'Comparison between V interpolated to 1/16 sec from 1 min, and V at 1/16 sec from SPICE'
...
probe = 'a'
ndim = 3
for ii=0,ndim-1 do check_hires_v_gsm, date, probe=probe, component_index=ii
end