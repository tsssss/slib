;+
; Check if there is a systematic time tag offset of B E and spice_var.
;-
...
pro test_perigee_time_tag_offset_in_b_e_and_spice, time_range, probe=probe, b_offset=b_offset, spice_offset=spice_offset, e_offset=e_offset
...
;---Load basic data.
    ...
    rbsp_read_spice_var, time_range, probe=probe, time_tag_offset=spice_offset
...
;---Get E and B in mGSE manually.
    foreach type, ['e','b'] do begin
        ...
        if type eq 'e' then begin
            vec_uvw = sinterpol(vec_uvw, times+e_offset, times, quadratic=0, /nan)
        endif
...
;---Calc E model.
    ...
    label = 'dT (sec)'
    var = prefix+'de_mgse'
    if spice_offset eq 0 then begin
        msg = '0'
    endif else begin
        msg = '1/'+string(1/abs(spice_offset),format='(I0)')
        if spice_offset lt 0 then msg = '-'+msg
    endelse
    label += '!C  r='+msg
    var += '!Cr='+msg
    ...
    label += '!C  b='+msg
    var += '!Cb='+msg
    if e_offset eq 0 then begin
        msg = '0'
    endif else begin
        msg = '1/'+string(1/abs(e_offset),format='(I0)')
        if e_offset lt 0 then msg = '-'+msg
    endelse
    label += '!C  e='+msg
    var += '!Ce='+msg
...
b_offsets = [0,-1d/32]
;b_offsets = [0,-1d/64-1d/128]

spice_offsets = [0,[-1,1]*0.03125]
spice_offsets = [0,[-1,1]*1d/64]
spice_offsets = [0]

e_offsets = [0,[-1,1]*1d/128]

foreach e_offset, e_offsets do begin
    foreach b_offset, b_offsets do begin
        foreach spice_offset, spice_offsets do begin
            test_perigee_time_tag_offset_in_b_e_and_spice, time_range, probe=probe, $
                b_offset=b_offset, spice_offset=spice_offset, e_offset=e_offset
        endforeach
    endforeach
endforeach
prefix = 'rbsp'+probe+'_'
