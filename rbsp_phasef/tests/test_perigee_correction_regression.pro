...
; Use regression to figure out the rotation angles.
;-


pro test_perigee_correction_regression, test_time, probe=probe
...
;---Get u_mgse = v_mgse-vcoro_mgse.
    ...
    yy1 = de_mgse[*,1]
    ax = u_mgse[*,0]*b_mgse[*,1]
    ay = -(u_mgse[*,0]*b_mgse[*,0]+u_mgse[*,2]*b_mgse[*,2])
    az = u_mgse[*,2]*b_mgse[*,1]
    bx = u_mgse[*,1]*b_mgse[*,0]
    by = u_mgse[*,1]*b_mgse[*,1]
    bz = u_mgse[*,1]*b_mgse[*,2]
    xx1 = [transpose(ax),transpose(ay),transpose(az), $
        transpose(bx),transpose(by),transpose(bz)]
    xx1 = [transpose(ax),transpose(ay),transpose(az)]
    res1 = regress(xx1,yy1, sigma=sigma, const=const1, measure_errors=measure_errors)
    print, res1
    
    yy2 = de_mgse[*,2]
    ax = u_mgse[*,0]*b_mgse[*,2]
    ay = u_mgse[*,1]*b_mgse[*,2]
    az = -(u_mgse[*,0]*b_mgse[*,0]+u_mgse[*,1]*b_mgse[*,1])
    bx = u_mgse[*,2]*b_mgse[*,0]
    by = u_mgse[*,2]*b_mgse[*,1]
    bz = u_mgse[*,2]*b_mgse[*,2]
    xx2 = [transpose(ax),transpose(ay),transpose(az), $
        transpose(bx),transpose(by),transpose(bz)]
    xx2 = [transpose(ax),transpose(ay),transpose(az)]
    res2 = regress(xx2,yy2, sigma=sigma, const=const2, measure_errors=measure_errors)
    print, res2
    ...
    omega_mgse = (fltarr(ntime)+1) # res
    de1_mgse = vec_cross(u_mgse,-vec_cross(b_mgse,omega_mgse))
    
    times = common_times[time_index]
    store_data, prefix+'dey', times, [[yy1],[reform(xx1 ## res1)]], limits={$
        ytitle:'(mV/m)', labels:['dEy','fit'], colors:sgcolor(['blue','red'])}
    store_data, prefix+'dez', times, [[yy2],[reform(xx2 ## res2)]], limits={$
        ytitle:'(mV/m)', labels:['dEz','fit'], colors:sgcolor(['blue','red'])}
    stop
...
foreach probe, probes do foreach test_time, test_times do test_perigee_correction_regression, test_time, probe=probe
end
