...
;---Test for a year.
    ...
    ;---Read fit_list
        ...
        ; Remove those fit with large rotation angle.
        the_times = section_times[*,0]
        rad = constant('rad')
        for ii=0,2 do begin
            index = where(abs(y_coef[*,ii]) ge 5*rad)
            y_coef[index,*] = 0
            index = where(abs(z_coef[*,ii]) ge 5*rad)
            z_coef[index,*] = 0
        endfor
    ...
    ;---Fit Ey and Ez separately and together.
        ;if check_if_update(prefix+'ey_fit') then begin
            ...
            store_data, prefix+'ey_fit', times, ey
            store_data, prefix+'ez_fit', times, ez
        ;endif
    ...
    ;---Prepare plot.
        ...
        plot_file = join_path([root_dir,prefix+'ey_ez_before_after_fit_per_maneuver.pdf'])
