...
pro test_perigee_time_tag_offset_in_b_and_spice, time_range, probe=probe, b_offset=b_offset, spice_offset=spice_offset
...
;---Get E and B in mGSE manually.
    foreach type, ['e','b'] do begin
        ...
        if type eq 'b' then begin
            vec_uvw = sinterpol(vec_uvw, times+b_offset, times, /quadratic)
        endif
...
;---Calc E model.
    ...
    diff_var = prefix+'de_mgse_'+$
        strtrim(string(spice_offset,format='(F10.4)'),2)+'_'+$
        strtrim(string(b_offset,format='(F10.4)'),2)
...
spice_offsets = [0,-1d/64]
foreach b_offset, b_offsets do begin
    foreach spice_offset, spice_offsets do begin
        test_perigee_time_tag_offset_in_b_and_spice, time_range, probe=probe, $
            b_offset=b_offset, spice_offset=spice_offset
    endforeach
endforeach
prefix = 'rbsp'+probe+'_'
ylim, prefix+'de_mgse_*', [-1,1]*4
tplot, prefix+'de_mgse_*'
