;+
; Try to fix L2 esvy and re-calculate the perigee residue.
;-

;---Settings.
    time_range = time_double(['2014-06-14','2014-06-19'])
    time_range = time_double(['2014-06-18','2014-06-19'])
    probe = 'b'
...
;---Calculate E-E_coro-E_vxb.
    ...
    get_data, prefix+'efw_esvy', times
    print, 'Start and end time of '+time_string(time_range[0],tformat='YYYY-MM-DD')+', RBSP-'+strupcase(probe)
    print, time_string(minmax(times),tformat='YYYY-MM-DD/hh:mm:ss.ffffff')
    stop
    evar_old = prefix+'efw_esvy'
    get_data, evar_old, times, esvy
    ; Fix time tag.
    dtimes = times[1:-1]-times[0:-2]
    index = where(dtimes le 0)
    dt = median(dtimes)
    index = where(abs(dtimes) ge dt*5)
    times[index[0]+1:index[1]] += dt-dtimes[index[0]]
    ; Remove Ew.
    esvy[*,2] = 0
    store_data, evar_old, times, esvy
    options, evar_old, 'colors', constant('rgb')

    rbsp_efw_cal_waveform, probe=probe, datatype='esvy', trange=time_range, /noclean, coord='uvw'
