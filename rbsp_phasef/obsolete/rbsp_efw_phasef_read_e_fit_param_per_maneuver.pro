;+
; Read the fit results for e_fit per manuver.
;
; The fit is done using e_uvw, i.e., independent on the spinfit data.
;-

function rbsp_efw_phasef_read_e_fit_param_per_maneuver, probe=probe
...
;---Settings and constants.
    ...
    rbspx = 'rbsp'+probe
    root_dir = join_path([rbsp_efw_phasef_local_root(),'efw_phasef','e_fit',rbspx])
    if file_test(root_dir) eq 0 then file_mkdir, root_dir


    ; Load the time of the sections.
    section_times = rbsp_efw_phasef_read_e_fit_times(probe=probe)
    ...
    sav_file = join_path([root_dir, prefix+'e_fit_param_per_maneuver.sav'])
...
;        ; Removing eclipse results in insufficient data for the fit.
    ...
    if is_updated then begin
        ...
        log_file = join_path([root_dir, prefix+'e_fit_param_per_maneuver.txt'])
        tab = '    '
...
foreach probe, ['a','b'] do fit_list = rbsp_efw_phasef_read_e_fit_param_per_maneuver(probe=probe)
stop
