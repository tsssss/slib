;+
; Read the e_fit result for the optimum boom pair for spinfit.
;-

function rbsp_efw_phasef_read_e_fit_param, probe=probe
...
;---Settings and constants.
    ...
    sav_file = join_path([root_dir, prefix+'e_fit_param.sav'])
...
;---Load e_fit parameters per maneuver and modify if needed.
    fit_list = rbsp_efw_phasef_read_e_fit_param_per_maneuver(probe=probe)
    section_times = list()
    y_coef = list()
    z_coef = list()
...
;---Loop through the sections.
    ...
    log_file = join_path([root_dir, prefix+'e_fit_param.txt'])
    tab = '    '
...
foreach probe, ['a','b'] do fit_list = rbsp_efw_phasef_read_e_fit_param(probe=probe)
