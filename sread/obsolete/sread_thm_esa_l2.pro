...
function sread_thm_esa_l2, tr0, probes = probe0, filename = fn0, $
    ...
    if n_elements(var0s) eq 0 then begin
        ; peir:3sec, peib:burst mode, peif:full mode, high angular res, few min.
        tvar = ['time','density','velocity_gsm']
        var0s = pre0+['peir_'+tvar,'peer_'+tvar]
    endif
...
utr = time_double(['2014-08-27/09:30','2014-08-27/10:30'])
esa = sread_thm_esa_l2(utr, probes='d')

store_data, 'thd_vbulk', esa.thd_peir_time, esa.thd_peir_velocity_gsm, $
    limits={colors:[6,4,2], labels:['x','y','z']}
