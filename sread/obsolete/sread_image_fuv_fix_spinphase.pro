;+
; Type: function.
; Purpose: Fix spinphase of IMAGE FUV MLT image for a time range. 
...
; Return: dblarr[n]. The fixed spinphase in degree.
...
function sread_image_fuv_fix_spinphase, tr0, filename = fn0, $
    spin = spins, $
...
;    if nrec gt 10 then dat.spinphase = smooth(dat.spinphase,5)
    ...
    for i = 0, nrec-1 do begin
        ...
        nudge = 1
        while nudge do begin
            edge = where(mlat eq mlat[0] and mlt eq mlt[0], cnt)
            timg = bytscl(img,min=img[0]) & timg[edge] = 0
            tmlat = bytscl(mlat, /nan)
            tmlt = bytscl(mlt, /nan)
            xsz = (size(mlat,/dimensions))[0]
            ysz = (size(mlat,/dimensions))[1]
            sgopen, 0, xsize = xsz*2, ysize = ysz*2
            device, decomposed = 0
            loadct, 1
            tv, tmlat, 1, /nan
            xyouts, 0.5,0.5, 'MLat'
            ...
            xyouts, 0,0, 'MLT'
            tv, timg-tmlat, 0
            xyouts, 0.5,0, 'Raw image'
            tvscl, congrid(mltimg, xsz, ysz), 3, /nan
            xyouts, 0,0.5, 'MLT image'
            sgtruecolor
            sgset_map, position = [0.5,0,1,0.5], color = sgcolor('white')
            
            tval = ''
            read, tval, prompt = 'enter new spinphase? (y/n)'
            if tval eq 'n' then break
            tval = 0d
            read, tval, prompt = 'enter new spinphase: (current value '+string(spinphase)+')'
            spinphase = tval
...
utr = '2001-10-22/08:03:17'
wic = sread_image_fuv_fix_spinphase(utr, /half)
