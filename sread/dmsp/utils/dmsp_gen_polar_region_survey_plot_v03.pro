...
; v03 add ion_velosity. v02 uses the mlt from aacgm_mlon2mlt, v01 uses the mlt from mlon2mlt.
;-

function dmsp_gen_polar_region_survey_plot_v03, input_time_range, probe=probe, $
    plot_dir=plot_dir, position=full_pos, errmsg=errmsg, test=test, local_root=local_root
    ...
    if errmsg ne '' then return, retval
    ion_vel_var = dmsp_read_ion_vel(full_time_range, probe=probe, errmsg=errmsg)
    if errmsg ne '' then return, retval
    ...
    foreach time, times, time_id do begin
        ...
        fig_labels = letters(nvar)+') '+['e-','e-','H+','V']
    ...
    ;---Add vector for velocity.
        vel_xyz = get_var_data(ion_vel_var, in=the_time_range, times=vel_times)
        if n_elements(vel_xyz) ne 0 then begin
            ...
            sc_zs = fltarr(n_elements(sc_xs))
            sc_rs = [[sc_xs],[sc_ys],[sc_zs]]
            sc_rs = sinterpol(sc_rs, the_times, vel_times)
            x_hat = sunitvec(sinterpol((sc_rs[1:-1,*]-sc_rs[0:-2,*]),(vel_times[1:-1]+vel_times[0:-2])*0.5, vel_times))
            z_hat = fltarr(n_elements(x_hat[*,0]),3)
            z_hat[*,2] = -1
            index = where(interpol(mlats,the_times, vel_times) ge 0, count)
            if count ne 0 then z_hat[*,2] = 1
            ...
            foreach time, vel_times, tid do begin
                if tid mod 10 ne 0 then continue
                plots, [x0s[tid],x1s[tid]], [y0s[tid],y1s[tid]], color=vel_color
            endforeach
...
files = dmsp_gen_polar_region_survey_plot_v03(the_time_range, probe=probe, plot_dir=plot_dir, test=test)
...
foreach probe, probes do begin
    files.add, dmsp_gen_polar_region_survey_plot_v03(input_time_range, probe=probe, test=test), extract=1
endforeach
files = files.toarray()
end