...
function dmsp_load_ssusi, input_time_range, id=datatype, probe=probe, $
...
;---Check inputs.
    ...
    if n_elements(local_root) eq 0 then local_root = join_path([default_local_root(),'dmsp'])
...
;---Init settings.
    ...
    base_name = 'dmsp'+probe+'_ssusi_edr-aurora_%Y%jT%H.*.nc'
    local_path = [local_root,'dmsp'+probe,'ssusi','data','edr-aurora','%Y','%m%d']
    remote_path = [remote_root,'dmsp'+probe,'ssusi','data','edr-aurora','%Y','%j']
    type_dispatch['edr_aurora'] = dictionary($
        'pattern', dictionary($
...
;---Dispatch patterns.
    if n_elements(datatype) eq 0 then datatype = 'edr_aurora'
...
time_range = ['2013-05-01/07:00','2013-05-01/09:00']
probe = 'f18'
