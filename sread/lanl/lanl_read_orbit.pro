...
pro lanl_read_orbit, time_range, probe=probe, errmsg=errmsg, coord=coord
    ...
    if coord ne 'geo' then begin
        get_data, 'r_geo', times, r_geo
        r_vec = cotran(r_geo, times, 'geo2'+coord)
        
        var = pre0+'r_'+coord
        store_data, var, times, r_vec
