...
function polar_load_ssc, input_time_range, id=datatype, $
...
;---Init settings.
    ...
    ; This is a self-made data. pre and def are combined.
    valid_range = ['1996-02-27','2008-06-13/24:00']
    base_name = 'po_or_sheng_%Y%m%d_'+version+'.cdf'
    local_path = [local_root,'polar','orbit_sheng','%Y']
    type_dispatch['sheng'] = dictionary($
        'pattern', dictionary($
    ...
    ; This is also a self-made data. Gaps and data are filled and errors in ssc are corrected.
    valid_range = ['1996-02-27','2008-06-14/24:00']
    base_name = 'po_or_def_%Y%m%d_'+version+'.cdf'
    local_path = [local_root,'polar','orbit','%Y']
    type_dispatch['ssc'] = dictionary($
        'pattern', dictionary($
        'local_file', join_path([local_path,base_name]), $
        'local_index_file', join_path([local_path,default_index_file()])), $
        'valid_range', time_double(valid_range), $
...
files = polar_load_ssc(time_range)
end
