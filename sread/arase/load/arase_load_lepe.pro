...
function arase_load_lepe, input_time_range, probe=probe, id=datatype, $
...
;---Check inputs.
    ...
    if n_elements(version) eq 0 then version = 'v[0-9_.]{5}'
    if n_elements(datatype) eq 0 then datatype = 'l3%pa'
...
;---Init settings.
    ...
    ; Level 3.
    base_name = 'erg_lepe_l3_pa_%Y%m%d_'+version+'.cdf'
    remote_path = [remote_root,'lepe','l3','pa','%Y','%m']
    local_path = [local_root,'lepe','l3','pa','%Y','%m']
    ...
    base_name = 'erg_lepe_l3_pa_fine_%Y%m%d_'+version+'.cdf'
    remote_path = [remote_root,'lepe','l3','pa_fine','%Y','%m']
    local_path = [local_root,'lepe','l3','pa_fine','%Y','%m']
    type_dispatch['l3%pa_fine'] = dictionary($
        'pattern', dictionary($
            'local_file', join_path([local_path,base_name]), $
            'local_index_file', join_path([local_path,default_index_file(/sync)]), $
            'remote_file', join_path([remote_path,base_name]), $
    ...
    ; Level 2.
    base_name = ''
    base_name = 'erg_lepe_l2_3dflux_%Y%m%d_'+version+'.cdf'
    remote_path = [remote_root,'lepe','l2','3dflux','%Y','%m']
    local_path = [local_root,'lepe','l2','3dflux','%Y','%m']
    type_dispatch['l2%3dflux'] = dictionary($
        'pattern', dictionary($
            'local_file', join_path([local_path,base_name]), $
            'local_index_file', join_path([local_path,default_index_file(/sync)]), $
            'remote_file', join_path([remote_path,base_name]), $
...
print, arase_load_lepe(time_range, id='l2%3dflux', probe=probe)
print, arase_load_lepe(time_range, id='l3%pa', probe=probe)
print, arase_load_lepe(time_range, id='l3%pa_fine', probe=probe)
end