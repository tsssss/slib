...
function arase_read_en_spec, input_time_range, probe=probe, errmsg=errmsg, $
    ...
    if species eq 'e' then begin
        files = arase_load_lepe(time_range, id='l3%pa', probe=probe, errmsg=errmsg)
        if errmsg ne '' then return, retval
    endif else begin
        files = arase_load_lepi(time_range, id='l3%pa', probe=probe, errmsg=errmsg)
        if errmsg ne '' then return, retval
    endelse
    ...
    pitch_angle_var = (species eq 'e')? flux_var+'_Alpha': 'FIDU_Alpha'
    pitch_angles = cdf_read_var(pitch_angle_var, filename=files[0])
...
;---Treat pa.
    ...
    if species eq 'e' then begin
        energys = reform(sqrt(energys[*,0,*]*energys[*,1,*]))
        fluxs *= 1e3    ; lepe unit: #/s-cm2-sr-eV; lepi unit: #/s-cm2-sr-keV/q
    endif else begin
        energys *= 1e3  ; lepe energy: eV; lepi energy: keV.
    endelse
...
vars = list()
vars.add, arase_read_en_spec(time_range, species='e')
vars.add, arase_read_en_spec(time_range, species='o')
vars.add, arase_read_en_spec(time_range, species='p')
vars = vars.toarray()
