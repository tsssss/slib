...
function goes_read_kev_electron, input_time_range, probe=probe, $
    ...
    if keyword_set(spec) then begin
        ...
        options, var, 'zlog', 1
        options, var, 'ylog', 1
        options, var, 'ytitle', 'Energy ('+energy_unit+')'
        options, var, 'ztitle', short_name+' ('+flux_unit+')'
        if n_elements(energy_bins) ne 0 then begin
            ylim, var, min(energy_bins), max(energy_bins)
        endif
    endif
