;+
; Type: function.
; Purpose: Read THEMIS E field data.
...
; Notes: none.
; History:
;   2018-02-24, Sheng Tian, create.
;-

function sread_thm_vaf_l1, tr0, probes = probe0, filename = fn0, $
...
;---prepare file names.
    type1 = 'vaf'   ; use in filename.
    type2 = 'vaf'   ; use in path.
    prb = (n_elements(probe0))? probe0: 'a'
    vsn = (n_elements(version))? version: 'v[0-9]{2}'
    ext = 'cdf'

    baseptns = ['th'+prb+'_l1_'+type1,'_YYYYMMDD_',vsn+'.'+ext]
    nbaseptn = n_elements(baseptns)
    ptnflags = [0,1,0]
    rempaths = [remroot+'/th'+prb+'/l1/'+type1,'YYYY',baseptns]
    locpaths = [locroot+'/th'+prb+'/l1/'+type1,'YYYY',baseptns]
    ...
    if keyword_set(download_only) then return, 1

;---prepare var names.
    pre0 = 'th'+prb+'_'
    if n_elements(var0s) eq 0 then begin
        var0s = pre0+['vaf','vaf_time']   ; 1/8 sec resolution.
    endif
