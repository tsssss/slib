...
;-
function download_file_callback_time2string, sec0
    time = sec0
    sec = time mod 60
    time = floor(time/60)
    min = time mod 60
...
function download_file_callback, status, progress, data
    ...
    if duration ge tstep then begin
        ...
        msg = string(data.fsize_string, $
            ...
            download_file_callback_time2string(tspent), $
            download_file_callback_time2string(tleft), $
            format=format)
...
pro download_file, local_file, remote_file, errmsg=errmsg
    ...
    oo->setproperty, callback_function='download_file_callback', $
        callback_data={ $
