...
; var_list=. A list of dictionary containing the following info:
...
;       Optional, out_vars are in_vars by default.
;    time_var_name. A string. The name of the time variable in files. Optional,
;       will find depend_0 if it is not set.
;    time_var_type. A string sets the type of time for the time variable. For
;      example: 'epoch', 'epoc16', etc. Optional, by default is 'unix'.
;    generic_time. A boolean. Set it to interpret time, time_var_name, times as
;      a generic independent variable and omit time_var_type.
...
pro read_vars, input_time_range, files=files, var_list=var_lists, errmsg=errmsg
    ...
    ;---Check if there is a time var.
        ...
        if n_elements(time_var_name) eq 0 then begin
            info = cdf_read_setting(in_vars[0], filename=files[0])
            if info.haskey('depend_0') then time_var_name = info['depend_0']
        endif
    ...
    ;---Find rec_infos using time info.
        ...
        check_rec_info = n_elements(time_range) ne 0 and has_time_var
        if check_rec_info then begin
            ...
            if ~keyword_set(generic_time) then begin
                ...
                time_info = convert_time(time_range, from=time_type, to=time_var_type)
