function read_var_from_file_per_var, var, file=data_file, errmsg=errmsg, _extra=ex

    cdf_load_var, var, filename=data_file, errmsg=errmsg, _extra=ex
    if errmsg ne '' then return, 0 else return, 1

end

function read_var_from_file, var_info, file=data_file, errmsg=errmsg, _extra=ex
    ...
    if var_type eq 7 then begin
        ...
        foreach var, var_info do begin
            tmp = read_var_from_file_per_var(var, file=data_file, errmsg=errmsg, _extra=ex)
            if errmsg ne '' then break
        endforeach
