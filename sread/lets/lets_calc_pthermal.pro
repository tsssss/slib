...
function lets_calc_pthermal, n_var=n_var, t_var=t_var, $
    var_info=var_info, $
    ...
    if n_elements(var_info) eq 0 then begin
        var_info = strlowcase(get_prefix(n_var)+'pthermal')
    endif
    time_range = get_var_setting(n_var, 'requested_time_range')
    ...
    dens = get_var_data(n_var, times=times, settings=settings)
    temp = get_var_data(t_var, at=times)
    pthermal = calc_thermal_pressure(dens, temp)
