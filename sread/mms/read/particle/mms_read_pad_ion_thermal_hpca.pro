...
function mms_read_pad_ion_thermal_hpca, input_time_range, id=datatype, probe=probe, species=species, $
    ...
    ; Load files.
    files = mms_ld_hpca_pad_ion(time_range, probe=probe, errmsg=errmsg)
    if errmsg ne '' then return, retval


;---Read data.
    var_list = list()
    in_vars = [prefix+'pad_hpca_'+species]
    out_vars = [prefix+'pad_'+species+'_thermal'+suffix]
...
ion_var = mms_read_pad_ion_thermal(tr, probe=probe, species='p', id='fpi')
stop
ion_var = mms_read_pad_ion_thermal(tr, probe=probe, species='p')
stop
