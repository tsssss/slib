;+
; Read ion energy spectrogram.
; species=. ['p','o','he','alpha']
; id=. ['all','kev','thermal']
; instrument=. ['fpi','hpca'] for id='thermal'.
;-

function mms_read_en_spec_ion, input_time_range, id=datatype, probe=probe, species=species, var_info=var_info, $
    ...
    var_info = pad_get_en_spec(pad_var=pad_var, pitch_angle_range=pitch_angle_range, var_info=var_info)
    return, var_info
...
foreach species, ion_species do plot_vars.add, mms_read_en_spec_ion(tr, probe=probe, species=species, id='thermal', suffix='_hpca', instrument='hpca')
