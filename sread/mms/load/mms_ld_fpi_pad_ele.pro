...
function mms_ld_fpi_pad_ele_gen_file, input_time_range, probe=probe, filename=cdf_file, errmsg=errmsg
...
function mms_ld_fpi_pad_ele, input_time_range, id=datatype, probe=probe, $
...
;---Find files, read variables, and store them in memory.
    ...
    if n_elements(nonexist_files) ne 0 then begin
        foreach file, request.nonexist_files do begin
            ...
            local_file = mms_ld_fpi_pad_ele_gen_file(file_time, filename=local_file, probe=probe)
        endforeach
...
probe = '1'
file = join_path([homedir(),'mms_ld_pad_fpi_ele_test_file.cdf'])
file = mms_ld_fpi_pad_ele_gen_file(tr, probe=probe, filename=file)
stop
files = mms_ld_fpi_pad_ele(tr, probe=probe)
end