...
function mms_ld_edp, input_time_range, id=datatype, probe=probe, $
...
;---L2
    ...
    foreach type_str, types do begin
        keys = [level_str,mode_str,type_str]
        the_key = strjoin(keys,'%')
        if type_str eq 'dce' then date_str = '%Y%m%d' else date_str = '%Y%m%d000000'
        base_name = mission_str+probe+'_'+instr_str+'_'+mode_str+'_'+level_str+'_'+type_str+'_'+date_str+'_'+version+'.cdf'
        local_path = [local_root,mission_str+probe,instr_str,mode_str,level_str,type_str,'%Y','%m']
        remote_path = [remote_root,mission_str+probe,instr_str,mode_str,level_str,type_str,'%Y','%m']
        type_dispatch[the_key] = dictionary($
            'pattern', dictionary($
...
probe = '1'
;files = mms_ld_edp(time_range, probe=probe, id='l2%slow%scpot')
files = mms_ld_edp(time_range, probe=probe, id='l2%slow%dce')
end