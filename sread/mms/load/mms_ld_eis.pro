...
function mms_ld_eis, input_time_range, id=datatype, probe=probe, $
...
;---L2
    ...
    types = ['extof','electronenergy','partenergy','phxtof']
    ...
    mode_str = 'brst'
    valid_range = mms_valid_range([instr_str,level_str,mode_str], probe=probe)
    types = ['extof','electronenergy','phxtof']
    foreach type_str, types do begin
        keys = [level_str,mode_str,type_str]
        the_key = strjoin(keys,'%')
        base_name = mission_str+probe+'_'+instr_str+'_'+mode_str+'_'+level_str+'_'+type_str+'_%Y%m%d_'+version+'.cdf'
...
probe = '2'
files = mms_ld_eis(data_tr, probe=probe, id='l2%srvy%extof')
end