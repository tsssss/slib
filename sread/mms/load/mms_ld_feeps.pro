...
function mms_ld_feeps, input_time_range, id=datatype, probe=probe, $
...
;---L2
    ...
    foreach type_str, species do begin
        keys = [level_str,mode_str,type_str]
        the_key = strjoin(keys,'%')
        base_name = mission_str+probe+'_'+instr_str+'_'+mode_str+'_'+level_str+'_'+type_str+'_%Y%m%d000000_'+version+'.cdf'
    ...
    foreach type_str, species do begin
        keys = [level_str,mode_str,type_str]
        the_key = strjoin(keys,'%')
        base_name = mission_str+probe+'_'+instr_str+'_'+mode_str+'_'+level_str+'_'+type_str+'_%Y%m%d%H%M%S_'+version+'.cdf'
        local_path = [local_root,mission_str+probe,instr_str,mode_str,level_str,type_str,'%Y','%m']
        remote_path = [remote_root,mission_str+probe,instr_str,mode_str,level_str,type_str,'%Y','%m']
        type_dispatch[the_key] = dictionary($
            'pattern', dictionary($
...
probe = '1'
files = mms_ld_feeps(time_range, probe=probe, id='l2%srvy%ion')
end