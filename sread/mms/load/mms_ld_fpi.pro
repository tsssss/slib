...
function mms_ld_fpi, input_time_range, id=datatype, probe=probe, $
...
;---L2
    ...
    types = ['dist','distaux','moms','momsaux','partmoms']
    types = ['des-'+types,'dis-'+types]
...
;---Find files, read variables, and store them in memory.
    files = prepare_files(request=request, errmsg=errmsg, local_files=files, $
        file_times=file_times, time=time_range, nonexist_files=nonexist_files, get_all_match=1)
...
probe = '2'
files = mms_ld_fpi(data_tr, probe=probe, id='l2%fast%des-moms')
end