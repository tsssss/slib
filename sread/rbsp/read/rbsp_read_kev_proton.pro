...
;---Read data.
    ...
    var = prefix+'kev_p_flux'
    enbins = cdf_read_var('FPDU_Energy', filename=files[0])
    ...
    ; filter pitch angle.
    pabins = cdf_read_var('FPDU_Alpha', filename=files[0])
    ...
    if keyword_set(spec) then begin
        ...
        options, var, 'ztitle', 'H!U+!N flux (#/cm!U2!N-s-sr-keV)'
    endif
...
var = rbsp_read_kev_proton(time_range, probe='b', energy=[0,300])
end
