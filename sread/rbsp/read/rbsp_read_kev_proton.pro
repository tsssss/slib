...
;---Read data.
    ...
    energy_bins = cdf_read_var('FPDU_Energy', filename=files[0])
    ...
    get_data, 'FPDU', common_times, fluxs
...
;---Apply pitch angle range.
    pitch_angle_bins = cdf_read_var('FPDU_Alpha', filename=files[0])
...
var = rbsp_read_kev_proton(time_range, probe='b', spec=1)
end
