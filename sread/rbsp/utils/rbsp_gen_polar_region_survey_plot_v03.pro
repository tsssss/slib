...
function rbsp_gen_polar_region_survey_plot_v03, input_time_range, probe=probe, $
    ...
    ; Load data.
    prefix = 'rbsp'+probe+'_'
    b_var = rbsp_read_bfield(time_range, probe=probe, coord='sm', errmsg=errmsg)
    if errmsg ne '' then return, retval
    e_var = rbsp_read_efield(time_range, probe=probe, resolution='survey', errmsg=errmsg)
    if errmsg ne '' then return, retval
...
;    n_var = rbsp_read_density(time_range, probe=probe)
    ...
    ; Euv.
    get_data, prefix+'e_uvw', times, e_uvw
    eu_var = prefix+'eu'
...
;---Plot settings.
    ...
    vars = [e_var,eu_var,ev_var,vsc_var,e_en_var,p_en_var,o_en_vars, $
        bmod_var+'_theta',dtheta_var,fmlat_var,mlat_var,mlt_var,dis_var]
    nvar = n_elements(vars)
    fig_labels = letters(nvar)+') '+$
        ['RB-'+strupcase(probe)+' E','Eu','Ev','Vsc','e-','H+','O+ para','O+ anti', $
        'Tilt','dTilt','f/MLat','MLat','MLT','|R|']
    ypans = [1,[1,1,0.8]*0.7,1,1,1,1, [0.8d,1,0.8,0.8,1,0.8]*0.7]
...
input_time_range = ['2016-06-23','2019-09-01']
input_time_range = ['2013-01-01','2019-09-01']
probes = ['a','b']
local_root = join_path([default_local_root(),'survey_plot','rbsp'])
...
foreach day, days do begin
    ...
    foreach probe, probes do begin
        ...
        files = rbsp_gen_polar_region_survey_plot_v03(the_time_range, probe=probe, plot_dir=plot_dir, xpansize=6, test=test)
    endforeach
endforeach
stop
...
files = list()
foreach probe, probes do begin
    files.add, rbsp_gen_polar_region_survey_plot(input_time_range, probe=probe, test=test), extract=1
endforeach
files = files.toarray()
end