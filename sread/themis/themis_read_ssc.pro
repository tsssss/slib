...
pro themis_read_ssc, time, datatype, probe=probe, level=level, $
    ...
    type_dispatch = [type_dispatch, $
        {id: 'pos@gsm', $
        base_pattern: thx+'_or_ssc_%Y%m01_'+version+'.cdf', $
        remote_pattern: join_path([rem_root,thx,'ssc','%Y']), $
        local_pattern: join_path([loc_root,thx,'ssc','%Y']), $
        variable: ['Epoch','XYZ_'+strupcase(coord)], $
        time_var: 'Epoch', $
        time_type: 'epoch'}]
    ...
    ; dispatch patterns.
    if n_elements(id) eq 0 then id = datatype
    ids = type_dispatch.id
    ...
    file_cadence = 'month'   ; 1 month for THEMIS.
...
themis_read_ssc, utr0, 'pos@gsm', 'd'
