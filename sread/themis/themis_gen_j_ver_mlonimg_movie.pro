;+
; Generate a movie for vertical currents on mlon/mlat.
;-

pro themis_gen_j_ver_mlonimg_movie, time_range, filename=movie_file, $
    ...
    themis_read_current_mlonimg, time_range
...
;---Gen image.
    ...
    foreach time, times, time_id do begin
        fig_file = join_path([fig_dir,'fig_thg_j_ver_mlonimg_'+time_string(time,tformat='YYYY_MMDD_hhmm_ss')+'.png'])
        if keyword_set(test) then fig_file = 0
        sgopen, fig_file, xsize=xsize, ysize=ysize

        timg = bytscl(reform(mlonimg[time_id,*,*]), min=zrange[0], max=zrange[1])
...
themis_gen_j_ver_mlonimg_movie, time_range, filename=test_file
end
