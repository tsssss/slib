...
function themis_asf_load_mlon_image_rect_per_site, input_time_range, site=site, id=datatype, $
...
;---Find files, read variables, and store them in memory.
    ...
    if n_elements(nonexist_files) ne 0 then begin
        foreach file, request.nonexist_files do begin
            ...
            themis_asf_load_mlon_image_rect_per_site_gen_file, file_time, site=site, filename=local_file, calibration_method=calibration_method
        endforeach
        files = prepare_files(request=request, errmsg=errmsg, local_files=files, $
            file_times=file_times, time=time_range, nonexist_files=nonexist_files)
    endif
...
foreach site, sites do files = themis_asf_load_mlon_image_rect_per_site(time_range, site=site)
end
