...
pro themis_asf_load_glon_image_per_site_gen_file, time, site=site, filename=file, errmsg=errmsg
...
;---Load GLon image and calibrate the count.
;   Calibration needs a longer time range to be perfect.
    ...
    themis_calc_asf_glon_image_per_site, asf_cal_var, errmsg=errmsg
    if errmsg ne '' then return
    
;    ; Calibrate brightness after mapping runs much faster.
;    themis_calc_asf_glon_image_per_site, asf_var, errmsg=errmsg
;    glon_image_var = 'thg_'+site+'_glon_image'
;    themis_asi_cal_brightness, glon_image_var, newname=glon_image_var
;    if errmsg ne '' then return
    ...
    times = times[index]
    glon_images = float(glon_images[index,*,*])
...
;---Save data.
    ...
    gatts = dictionary($
        'text', 'THEMIS ASI full-res images converted to GLon-GLat plane and background removed. Generated by Sheng Tian, ts0110@atmos.ucla.edu' )
    ...
    vatts = dictionary($
        ...
        'depend_0', time_var )
    cdf_save_var, glon_image_var, filename=file, value=glon_images
    cdf_save_setting, vatts, varname=glon_image_var, filename=file

    var = 'pixel_glon'
    val = float(lim.pixel_glon)
    vatts = dictionary($
        'var_type', 'metadata', $
        'unit', 'deg', $
        'var_notes', 'glon of each pixel' )
    cdf_save_var, var, filename=file, value=val, save_as_one=1
    cdf_save_setting, vatts, varname=var, filename=file

    var = 'pixel_glat'
    val = float(lim.pixel_glat)
    vatts = dictionary($
        'var_type', 'metadata', $
        'unit', 'deg', $
        'var_notes', 'glat of each pixel' )
    cdf_save_var, var, filename=file, value=val, save_as_one=1
    cdf_save_setting, vatts, varname=var, filename=file
...
file = join_path([homedir(),'test','test_glon_image.cdf'])
themis_asf_load_glon_image_per_site_gen_file, time_range, site=site, filename=file, errmsg=errmsg
end
