...
function glon_image_map_old2new, old_images, site=site, crop=crop

    input_time_range = [0d,0]
    themis_read_asf_glon_image_read_mapping_info, input_time_range, site=site
...
imgs = get_var_data('thg_'+site+'_asf', times=times)

crop = 1
; mean, 127 sec.
; median, 113 sec.
imgs_raw = glon_image_map_old2new(imgs, site=site, crop=crop)
stop
foreach time, times, time_id do sgtv, rotate(bytscl(reform(imgs_raw[time_id,*,*]),top=254, max=20000, min=0),5), ct=49
end
