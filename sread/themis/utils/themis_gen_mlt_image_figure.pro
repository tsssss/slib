;+
; Plot mlt image for given mlt image.
;
; mlt_image_var. A string of the tplot var of mlt images.
; fig_dir=fig_dir. Required. The directory to save the figures.
; time_step=. The time step for generating the figures. By default is that of the mlt images. Can set it to multiple times of the default time step.
;-
...
;---Get the cropping solution.
    if n_elements(crop_method) eq 0 then crop_method = 'keep_nightside'
    if crop_method eq 'keep_nightside' then begin
        mlt_range = [-6,6]
        mlat_range = lim.mlat_range
    endif else if crop_method eq 'auto' then begin
        flags = reform(mlt_images[0,*,*])
        for ii=1,ntime-1 do begin
            flags += reform(mlt_images[ii,*,*])
        endfor
        ...
        mlt_range = minmax(lim.pixel_mlt[index])
        mlat_range = minmax(lim.pixel_mlat[index])
        mlt_range = [floor(mlt_range[0]),ceil(mlt_range[1])]
;        mlat_step = 5d
;        mlat_range = [floor(mlat_range[0]/mlat_step)*mlat_step,ceil(mlat_range[1]/mlat_step)*mlat_step]
    endif
    if n_elements(mlat_range0) eq 2 then mlat_range = mlat_range0
    if n_elements(mlt_range0) eq 2 then mlt_range = mlt_range0

    pixel_mlt = lim.pixel_mlt
    pixel_mlat = lim.pixel_mlat
    index = where(pixel_mlt ge mlt_range[0] and pixel_mlt le mlt_range[1] $
    ...
    mlt_step = 2d
    mlt_tickv = make_bins(mlt_range, mlt_step, inner=1)
    tt_tickv = (mlt_tickv*15-90)*constant('rad')
    mlat_step = 10d
...
;test = 1
    ...
    ;---Draw image.
        ...
        foreach tt, tt_tickv, ii do begin
            ...
            ty = ty[index]
            msg = mlt_tickv[ii]
            if msg lt 0 then msg += 24
            msg = string(msg,format='(I02)')
