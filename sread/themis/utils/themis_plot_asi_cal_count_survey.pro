;+
; Plot the cal count of the image at the center time and selective pixels at all times.
;
...
;-
pro themis_plot_asi_cal_count_survey, test_time, $
    site=site, pixels=pixels, $
    pixel_labels=pixel_labels, filename=plot_file, test=test


;---Load data.
    if n_elements(site) eq 0 then return
    if n_elements(test_time) eq 0 then return
...
;---Settings.
    ...
    ypads = []
    ypans = []
    npan = n_elements(pixel_labels)
    nrow += 1+npan
    nletter += 2
    ypads = [ypads,0.4+fltarr(npan)]
...
event_list.add, dictionary($
    'site', 'snkq', $
    'time', '2015-10-05/09:57:30', $
    'pixels', list([140d,50]), $
    'pixel_labels', ['streamer'] )
;event_list.add, dictionary($
;    'site', 'inuv', $
...
test = 0
plot_dir = join_path([homedir(),'tmp'])

foreach event, event_list do begin
    plot_file = join_path([plot_dir,'fig_themis_plot_asi_cal_count_survey_'+time_string(event.time,tformat='YYYY_MMDD_hhmm')+'.pdf'])
    themis_plot_asi_cal_count_survey, event.time, pixels=event.pixels, pixel_labels=event.pixel_labels, site=event.site, filename=plot_file, test=test
endforeach
end
