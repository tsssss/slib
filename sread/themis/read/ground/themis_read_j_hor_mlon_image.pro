;+
; Read horizontal currents.
;-

function themis_read_j_hor_mlon_image, input_time_range, errmsg=errmsg, get_name=get_name

    errmsg = ''
    time_range = time_double(input_time_range)
    mlon_image_var = 'thg_j_hor_mlon_image'
    ...
    if (ndim ne 3) and (ndim ne 4) then begin
        errmsg = 'Data in invalid dimensions ...'
        return, ''
    endif
    if dims[ndim-1] eq 3 then old_images = reform(old_images,[1,dims])
    ncomp = 2
...
;---Map original image to mlon image.
    ...
    nframe = n_elements(old_images)/old_1d_size/ncomp
    ...
    if reform_old_image then begin
        ...
        old_images = reform(old_images, [nframe,old_image_size, ncomp])
    endif

    new_image_size = get_var_data(prefix2+'new_image_size')
    new_1d_size = product(new_image_size)
    new_images = fltarr([nframe,new_image_size,ncomp])
    ...
    for comp_id=0,ncomp-1 do begin
        for time_id=0,nframe-1 do begin
            old_image = reform(old_images[time_id,*,*,comp_id],old_1d_size)
    ...
    ;            val = mean(val, nan=1)
            ...
            endforeach
            new_images[time_id,*,*,comp_id] = reform(new_image, new_image_size)
        endfor
    endfor
...
mlon_image_var = themis_read_j_hor_mlon_image(time_range)
end