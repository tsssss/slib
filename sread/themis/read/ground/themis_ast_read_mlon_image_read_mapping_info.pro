;+
; Get the info for mapping a raw AST image to the MLon-MLat plane. AST image already flattens the fish-eye perspective, but we need to coerce it to the mlon image grids.
;-

pro themis_ast_read_mlon_image_read_mapping_info_gen_file, input_time_range, $
    site=site, filename=file

;---Get pixel info for AST image.
    ast_info = themis_asi_read_pixel_info(input_time_range, site=site, errmsg=errmsg, id='ast')
    ; Get the pixel's center position.
    center_elevs = ast_info.pixel_elev
    center_azims = ast_info.pixel_azim
    center_mlons = ast_info.pixel_mlon
    center_mlats = ast_info.pixel_mlat
    corner_mlons = ast_info.corner_mlon
    corner_mlats = ast_info.corner_mlat
...
;---Map old image to new image.
    ...
    foreach old_index_1d, old_uniq_pixels do begin
        ...
        pixel_mlons = corner_mlons[*,old_index_2d[0],old_index_2d[1]]
        pixel_mlats = corner_mlats[*,old_index_2d[0],old_index_2d[1]]
...
pro themis_ast_read_mlon_image_read_mapping_info, input_time_range, site=site
    ...
    base_name = 'thg_ast_mlon_image_mapping_info_'+site+'_'+version+'.tplot'
    ...
    if file_test(file) eq 0 then begin
        ...
        themis_ast_read_mlon_image_read_mapping_info_gen_file, input_time_range, site=site, filename=file
    endif
...
;    ast_info = themis_asi_read_pixel_info(time_range, site=site)
;    mlon_range.add, minmax(ast_info.pixel_mlon)
;    mlat_range.add, minmax(ast_info.pixel_mlat)
;endforeach
;
...
site = 'fsmi'
file = join_path([homedir(),'ast_mlon_image_metadata.tplot'])
themis_ast_read_mlon_image_read_mapping_info_gen_file, time_range, site=site, filename=file
end
