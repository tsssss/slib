;+
; Calculate moments by integrating a combined distribution of esa only.
; This is adopted from Jiang Liu's thm_load_esansst2
;
; Vectors returned are in themis_dsl.
;-

function themis_read_moment_combo_esa_integrate, input_time_range, probe=probe, id=id, $
...
;---Check input.
    ...
    prefix = 'th'+probe+'_'
    if n_elements(suffix) eq 0 then suffix = '_esa_integrate'
    ...
    ; id.
    if n_elements(id) eq 0 then id = 'esa'
