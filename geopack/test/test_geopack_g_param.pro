...
test_events.add, dictionary($
    ...
    'plot_base', 'geopack_g_test_qin_2007_fig1.pdf' )
test_events.add, dictionary($
    'time_range', time_double(['2000-308','2000-313'],tformat='YYYY-DOY'), $
    'plot_base', 'geopack_g_test_qin_2007_fig2.pdf' )
...
foreach test_event, test_events do begin
    ...
    param_vars = list()
    param_vars.add, geopack_read_g(time_range)
    param_vars.add, geopack_read_denton_g(time_range)
...
;            dtime = (jj eq 0)? 0: -1800
        ...
        endforeach
        param_str = 'g'+string(ii+1,format='(I0)')
        settings = dictionary($
            'display_type', 'stack', $
