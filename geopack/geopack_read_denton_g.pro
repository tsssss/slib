...
function geopack_read_denton_g, input_time_range, file=input_file, prefix=input_prefix, $
    ...
    ndim = n_elements(g_params[0,*])
    colors = get_color(ndim)
    settings = dictionary($
        'requested_time_range', time_range, $
        'display_type', 'stack', $
        'labels', 'G'+string(findgen(ndim)+1,format='(I0)'), $
        'colors', colors, $
        'ylog', 1, $
        'ytitle', 'G Param')
    tmp = var_store(var_info, g_params[time_index,*], times, settings=settings)
...
g_var = geopack_read_denton_g(time_range)
w_var = geopack_read_denton_w(time_range)
sgopen, 0, size=[6,4]
plot_vars = [g_var,w_var]
